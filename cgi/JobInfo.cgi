...
my $scriptname = "http://www.star.bnl.gov/cgi-bin/nikita/JobInfo2.cgi";
...
print 
    ...
    "<META HTTP-EQUIV=Refresh CONTENT=$updatetime>\n",
    "</HEAD>\n",
...
if( ($ProdTag) || ($Trigger) ){    
	...
	if( $#querystr>3 ){  
		...
		if( $chek=~/(cb)(\d+)/ ){
			...
			print "mv /star/u/starreco/$prodtag/requests/daq/archive/*$prodtag\_*$LFname 
	...
	if( $#querystr>3 ){                 
		...
		if( $chek=~/(cb)(\d+)/ ){ 
			...
			$sth1->execute($id);
			while( ($prodtag, $trigger, $LFname)= $sth1->fetchrow_array() ){ 
			    print MOVELIST "/star/u/starreco/$prodtag/requests/daq/archive/$prodtag.*$LFname 
                                                         /star/u/starreco/$prodtag/requests/daq/jobfiles/\n";
			}
		    }#if Method=CreatList
		    }        
...
#	print "<H4>Test sql syntax: $query</H4>\n";
	...
	print "\t</SELECT></td></tr>\n";
	print "<tr bgcolor=khaki><td>Trigger:</td><td bgcolor=beige><SELECT name=Trigger>\n",
	"<OPTION value=All>All\n";
