...
if ( $qperiod =~ /month/) {
 ...
 $nowdate = $todate;
  ...
  if($qprod eq "all2015"){
   ...
   $sql="SELECT DISTINCT date_format(starttime, '%Y-%m-%d') as SDATE FROM $ProdSizeT WHERE (prodtag = 'P15ic' or prodtag = 'P15ie' or prodtag = 'P15ik' or prodtag = 'P15il')  and date_format(starttime, '%Y-%m-%d') <> '0000-00-00' AND (TO_DAYS(\"$nowdate\") - TO_DAYS(starttime)) < ?  order by SDATE";
...
$ndt = 0;
  ...
  $sql="SELECT date_format(createtime, '%Y-%m-%d') as PDATE, sum(mudstsize) FROM $ProdSizeT WHERE (createTime BETWEEN '$tdate 00:00:00' AND '$tdate 23:59:59') and (prodtag = 'P15ic' or prodtag = 'P15ie' or prodtag = 'P15ik' or prodtag = 'P15il') group by PDATE  ";
...
$ndt = 0;
  ...
  $sql="SELECT date_format(starttime, '%Y-%m-%d') as PDATE, sum(daqsize) FROM $ProdSizeT WHERE  (starttime BETWEEN '$tdate 00:00:00' AND '$tdate 23:59:59') and (prodtag = 'P15ic' or prodtag = 'P15ie' or prodtag = 'P15ik' or prodtag = 'P15il')  group by PDATE  ";
