...
$sql="SELECT dataset, path, fName, createTime, Nevents, trigger, dataStatus, comment FROM $FileCatalogT where insertTime > '$thisDay'  AND fName like '%.event.root' AND type = 'daq_reco' and jobID like '%$prodPer%' AND site = 'disk_rcf' ORDER by insertTime ";
...
my $nstart = $ndbFiles - 101 ;

# foreach $eachFile (@dbFiles) {

for ($i = $nstart; $i<scalar(@dbFiles);$i++) {
...
print <<END;
  <html>
   <head>
          <title>Last 100 Files Inserted into FileCatalog</title>
   </head>
   <body BGCOLOR=\"#ccffff\"> 
     <h1 align=center>Last 100 Files Inserted into FileCatalog</h1>
