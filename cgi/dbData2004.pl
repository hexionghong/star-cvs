...
require "/afs/rhic.bnl.gov/star/users/didenko/cgi/dbProdSetup.pl";
...
my @detSet = ("all","tpc","svt","tof","ftpc","ssd","emc","eemc","bsmd","esmd","fpd","pmd");
my @trigSet;
my @mfield = ("all","HalfField","FullField","ReversedFullField","ReversedHalfField","FieldOff");
my @collis = ("AuAu200","AuAu62","AuAu19", "ProtonProton200","ProtonProton48" );
my @trigSet;
my $ntrigSet = 0;
my $mytrig;
my @locSet = ("hpss","disk");
...
print $query->header;
print $query->start_html('dbData2004');
print $query->startform(-action=>"dbProdSum2004.pl");  
