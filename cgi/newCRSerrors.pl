...
#!/usr/bin/env perl 
#
#  newCRSerrors.pl - script to display jobs failure due to different errors 
#
#####################################################################################
...
$ymax = 1;
  ...
  $gtitle = "Number of jobs failed per hour for the period of $fperiod ";
...
# $max_y = 140;
  ...
  $gtitle = "Number of failed jobs in % to number of jobs finished per hour for period of $fperiod ";
    ...
    $graph->set(x_label => "  ",
		...
		#labelclr => "lblack",
                titleclr => "lblack",
		dclrs => [ qw(lblue lred lgreen lpurple lblack lbrown marine lyellow lgrey lpink marine ) ],
		line_width => 2,
...
print <<END;
  <html>
   <head>
          <title>CRS jobs status (failure) </title>
   </head>
   <body BGCOLOR=\"#ccffff\">
