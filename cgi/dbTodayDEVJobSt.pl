...
$sql="SELECT path, logFile, jobStatus, NoEventDone, chainOpt, memUsageF, memUsageL, CPU_per_evt_sec, createTime FROM $JobStatusT where path LIKE '%/dev/%$testDay%' AND avail = 'Y' AND createTime like '$mdate%' ";
