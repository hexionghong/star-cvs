...
$crsJobStatusT = "crsJobStatusY11";
 ...
 $sql="SELECT DISTINCT  date_format(sdate, '%Y-%m-%d' ) as PDATE FROM $crsJobStatusT where sdate >= '2011-10-01 00:00:00' order by as PDATE " ;
...
#my $Nmaxjobs = 700;
 ...
 my $fperiod  =  $query->param('fsdate');
 my $lperiod  =  $query->param('lsdate');
 my $plview   =  $query->param('plotvw');


  if( $fperiod eq "" and $lperiod eq "" and $plview eq "" and $pryear eq "") {
...
my $qqr = new CGI;
 ...
 my $fperiod =  $qqr->param('fsdate');
 my $lperiod =  $qqr->param('lsdate');
