...
my $jobname = $qtrg."%".$qprod."%";
 ...
 }elsif($qflag eq "entryfc") {

   &beginFCHtml();

     $sql="SELECT jobfileName, jobStatus, NoEvents, createTime FROM $JobStatusT  where jobfileName like ? and prodSeries = ? and trigsetName = ? and jobStatus <> 'n/a' and outputFC <> 'yes'  ";
...
print <<END;
  ...
  <html>
   <body BGCOLOR=\"cornsilk\">
 <h2 ALIGN=CENTER> <B>List of jobs with missing MuDst entries in FileCatalog for<font color="blue"> $qprod </font> production <br> and <font color="blue">$qtrg </font> dataset  </B></h2>
 <h3 ALIGN=CENTER> Generated on $todate</h3>
<br>
<TABLE ALIGN=CENTER BORDER=5 CELLSPACING=1 CELLPADDING=2 bgcolor=\"#ffdc9f\">
<TR>
