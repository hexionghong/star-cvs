...
$day_diff = int($day_diff);
   ...
   $sql="SELECT DISTINCT date_format(submitTime, '%Y-%m-%d') AS PDATE  FROM $JobEfficiencyT WHERE ( lastKnownState = 'done' OR lastKnownState = 'failed' OR lastKnownState = 'killed' OR lastKnownState = 'held' ) AND prodType = 'reco' AND (TO_DAYS(\"$nowdate\") - TO_DAYS(submitTime)) < ? order by PDATE";
...
my $ndt = 0;
 ...
 $nstat = 0;
  ...
  if( $qsite eq "ALL" ) {
      ...
      $sql="SELECT date_format(submitTime, '%Y-%m-%d') AS PDATE, site, submitAttempt, globusError, dotOutHasSize, dotErrorHasSize, exec, transIn, transOut, overAllState FROM $JobEfficiencyT WHERE  submitTime like '$tdate%' AND (lastKnownState = 'done' OR lastKnownState = 'failed' OR lastKnownState = 'killed' OR lastKnownState = 'held' ) AND prodType = 'reco' "; 
    ...
    $cursor =$dbh->prepare($sql)
     ...
     $sql="SELECT date_format(submitTime, '%Y-%m-%d') AS PDATE, site, submitAttempt, globusError, dotOutHasSize, dotErrorHasSize, exec, transIn, transOut, overAllState FROM $JobEfficiencyT WHERE site = ? AND submitTime like '$tdate%' AND (lastKnownState = 'done' OR lastKnownState = 'failed' OR lastKnownState = 'killed' OR lastKnownState = 'held') AND prodType = 'reco' ";
