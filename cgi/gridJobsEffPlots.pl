...
my @prodyear = ("2006","2007","2008","2009","2010","2011","2012","2013","2014","2015");
...
my $tdate;
  ...
  if($pryear eq "2006") {
    ...
    $nowdate = "2010-12-31";
  }elsif($pryear eq "2011") {
    $nowdate = "2011-12-31";
  }elsif($pryear eq "2012") {
    $nowdate = "2012-12-31";
  }elsif($pryear eq "2013") {
    $nowdate = "2013-12-31";
  }elsif($pryear eq "2014") {
    $nowdate = "2013-12-31";
  }else{
    $nowdate = $todate;
  }
 ...
 $nstat = 0;
  ...
  if( $qsite eq "ALL" ) {
    ...
    $cursor =$dbh->prepare($sql)
     ...
     $sql="SELECT testday, site, globusStatus, globusError, logStatus, execStatus, transfer_in, transfer_out, recoFinishTime, createTime FROM $JobStatusT WHERE site = ? AND  testday = '$tdate' AND (status = 'complete' OR status = 'failed') ";
