...
#   print "Check parts  ", ,$prt[1],"  ",$prt[3],"\n";
  ...
  @jobhlist = `crs_job -stat | grep HELD` ;

    foreach my $jline (@jobhlist) {
     chop $jline ;
#     print $jline, "\n";
     @wrd = ();
     @wrd = split (" ", $jline);
     ...
     $jid[$mjob] = $wrd[0];
     $jid[$mjob] = substr($wrd[0],0,-1) + 0;


    @jobnode = ();
    @jobnode = `crs_job -long $jid[$mjob] | grep Machine`;

    @jobptag = ();
    @jobptag = `crs_job -long $jid[$mjob] | grep MuDst`;

    @jobStag = ();
    @jobStag = `crs_job -long $jid[$mjob] | grep Stdout:`;
...
#     print $jtag, "\n";
    ...
    if( defined($prt[6]) ) {
       ...
       $jbptag[$mjob] = $prt[6];
       $fname = $prt[10];
       $fname =~ s/.MuDst.root'//g ;       
       $jobname[$mjob] = basename($fname); 

    }else{

   foreach my $jjtag (@jobStag) {
     chop $jjtag ;
    if ( $jjtag =~ /prodlog/ ) {
       @prt = ();
       @prt = split("/", $jjtag) ;
       ...
       $jobFname[$mjob] = "%".$jbptag[$mjob]."_".$jobname[$mjob];

       print "Njob, jobID, obfileName, node for held job: ",$mjob,"   ", $jid[$mjob],"   ",$jobFname[$mjob],"   ", $jbnode[$mjob], "\n";      
...
if( $jbptag[$mjob] eq "P15ie" ) {
 ...
 $JobStatusT = "JobStatus2014";
  ...
  $JobStatusT = "JobStatus2015";
    ...
    $sql="update $JobStatusT set crsError = '$conderr', nodeID = '$jbnode[$mjob]', crsjobId = '$jid[$mjob]' where jobfileName like '$jobFname[$mjob]' ";

    $rv = $dbh->do($sql) || die $dbh->errstr;


  print "Job   ",$jid[$mjob],"   was reset due to condor problem","\n";

     `crs_job -reset $jid[$mjob]`;
     `crs_job -submit $jid[$mjob]`;
...
#    print "Job id = ",$jid[$njob], "\n";
    ...
    @jobptag = ();
    @jobptag = `crs_job -long $jid[$njob] | grep MuDst`;

    @jobStag = ();
    @jobStag = `crs_job -long $jid[$njob] | grep Stdout:`;
...
########
 ...
 $JOBDIR = "/star/u/starreco/".$jbptag[$njob]."/requests/daq/";
 $hpssfail = $JOBDIR."jobs_lostfiles";
 $resubdir = $JOBDIR."jobs_rerun";
...
#       print "Check line with filename : ", $prt[0],"  ", $prt[1],"\n";
       ...
       $jobFname[$njob] = "%".$jbptag[$njob]."_".$jobname[$njob];
       print "JobfileName for failed jobs   ", $jobFname[$njob], "\n";
...
if( $jbptag[$njob] eq "P15ie" ) {
 ...
 $JobStatusT = "JobStatus2014";
  ...
  print "Job   ",$jid[$njob],"   was reset due to I/O error","\n";
    ...
    print "Njob, Jobid, nodeID, input filename, error number are : ",$njob,"  ",$jid[$njob],"  ",$jbnode[$njob],"  ",$jobname[$njob],"   ",$Tperror,"\n";
