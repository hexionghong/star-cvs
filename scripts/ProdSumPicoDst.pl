...
# ProdSumPicoDst.pl
#
# Production summary from FileCatalog by stream data for MuDst -> picoDst conversion.
# 
################################################################################################
...
my $prodname = "n/a";
 ...
 $patt = $prod[$nlist].".".$libpico[$nlist] ;
   ...
   $fileC->clear_context( );
    ...
    $fileC->set_context("trgsetupname=$trig[$nlist]","production=$prod[$nlist]","library=$libpico[$nlist]","sname2=$nstreams[$kk]","filetype=daq_reco_picoDst","storage=HPSS","sanity=1","limit=0" );
   ...
   $fileC->set_context("trgsetupname=$trig[$nlist]","production=$prod[$nlist]","filetype=daq_reco_MuDst","sname2=$nstreams[$kk]","storage=HPSS","sanity=1","limit=0");
...
sub beginHtml {
 ...
 print HTML "<html>\n";
 print HTML " <head>\n";
 print HTML "          <title>Production MuDst -> picoDst Summary   </title>\n";
 print HTML "  </head>\n";
 print HTML "  <body BGCOLOR=\"cornsilk\">\n"; 
 print HTML "  <h2 ALIGN=CENTER> <B> MuDst -> picoDst Production Summary </B></h2>\n";
...
my $Date = `date`;
 ...
 print HTML "<!-- Created: February 9  11:58:00 -->\n";
