#!/opt/star/bin/perl
#
# $Id: swstat.pl,v 1.1 1999/09/16 16:37:10 wenaus Exp $
#
# $Log: swstat.pl,v $
...
$versions = `cd /afs/rhic/star/packages; ls -1 | grep SL`;
$versions =~ s/\n/ /g;
#@statfiles = ( 'SL99b', 'SL99d', 'SL99e', 'SL99f', 'SL99g', 'SL99h');
@statfiles = split (/ /,$versions);
%stattypes = ( 'star' => 'env',
              'root4star' => 'root'
              );
foreach $type ( sort keys %stattypes ) {
    foreach $e ( keys %starHpNodes ) { $starHpNodes{$e} = 0; }

foreach $file ( @statfiles ) {
    $filename = "/afs/rhic/star/group/statistics/$type$file";
    ...
    while (<FILE>) {
        ...
        $_ =~ s/asked for/in/;
        ($user, $dum1, $host, $dum2, $level, $dum4, $ver, $wk, $mo, $dy, $time, $zone, $yr) = 
            split / /;
        if ( ! ($user =~ m/^[a-z]+/) ) {next}  # a few with junk characters
        $level =~ s/STAR_LEVEL=//;
        if ( $level ne '.dev' && $level ne 'dev' && $level ne 'new' && $level ne 'pro' && $level ne 'old' && substr($level,0,2) ne 'SL' ) {next}
        $ver =~ s/STAR_VERSION=//;
        $host =~ s/\..*//;
