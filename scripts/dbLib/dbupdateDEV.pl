...
# get the directory list from output directories
...
# get the list of directories from output directories for beforday
...
#           Check the time bnd size before parsing the log
          	if( $ltime > 600 and $size > 1000 ) {
#   	    print "Log time: ", $ltime, "\n";
   		     print $fullname, "\n";

   		     next if($fullname =~ /hold/);
# Call the function logInfo
         	     ...
         	     ($$fObjAdr)->pyr($pyear);
      		     ...
      		     ($$fObjAdr)->avBtof($avr_btof);   # for btof
...
# Fill the JobStatus table
            ...
            } # this close the if function for time and size
            }
            else {  #if file is not log file
                    next;
            }
        } # end for loop for each file in output folder
    closedir DIR;
    }
}

# finished the parsing of logfiles of outfolder
...
foreach my $newjobFile (@testJobStFiles) {
    ...
    $avr_btof = ($$newjobFile)->avBtof; # for btof
...
#       print "File info: ",$idHash{$fullname}," % ", $jpath ," % ", $platf, " % ", $fullname, " % ", $geom, " % ", $EvType," % ", $EvGen, " % ", $EvReq," % ", $comp," % ", $EvDone," % ", $libV,  "\n";
                    ...
                    } # for each log file for loop ends
                    }
                 }
            }else { # if file does not contain root
                    next;
            }
        }
        closedir DIR;
        }
} # for each out directory ends
...
#	   if ($pfullName eq $thfullName) {
      ...
      } # for each old file for loop ends
} # for each test file for loop ends
...
} #for each new test filr fore loop ends
...
sub fillJSTable {
    ...
    $sql.="avg_no_btof='$avr_btof',"; # for btof
    $sql.="avail='$mavail'";
    print "$sql\n" if $debugOn;
    $rv = $dbh->do($sql) || die $dbh->errstr;
    $new_id = $dbh->{'mysql_insertid'};
...
#if log file name contains embed (embedding jobs)
if($fl_log =~ /embed/) {
...
foreach my $line (@logfile) {
    ...
    if ($line =~ /\(matched\)/)
    {   @part = split (" ", $line);
        $count_btof = $count_btof + 1;
        $total_btof = $total_btof + int($part[10]);
    }

    if($line =~ /Processing bfc.C/) {
          $runflag++;
	}
# root version and worker node where the job is executed
...
# check if job is completed
    ...
    } # if run=1 and emb =1 closed

} # for each line closed
...
if ($count_btof > 0)
{
    $avr_btof = $total_btof/$count_btof;
}
